...
function gsacp {
    ...
    git submodule foreach --quiet `
        "branch=\$(git symbolic-ref --short HEAD 2>/dev/null); if [ -z \"\$branch\" ]; then git checkout main 2>/dev/null; fi; git add . && git diff --cached --quiet || git commit -m '$msg' && git push" 2>$null
