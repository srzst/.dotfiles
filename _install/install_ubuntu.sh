...
# (symlink 생성 후 append 시 repo 파일이 직접 수정되는 문제 방지)
...
# FIX: PATH 갱신 후 gita 등록 (ensurepath만으론 현재 세션에 미반영)
pipx install gita
pipx ensurepath
export PATH="$HOME/.local/bin:$PATH"
gita add "$REPO" 2>/dev/null
echo "OK gita 설치 및 .dotfiles 등록 완료"
...
echo "OK remote URL HTTPS로 변경 완료 (GitHub Desktop 호환)"
